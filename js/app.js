(() => {
 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        function styles(name, s, value, remove) {
            if ("string" == typeof name && qe(`${name}`)) if (remove) return qe(`${name}`).style.removeProperty(`${remove}`); else return qe(`${name}`).style.cssText += `${s}: ${value}`; else if ("object" == typeof name) if (remove) return name.style.removeProperty(`${remove}`); else return name.style.cssText += `${s}: ${value}`;
        }
        ...
        function preloader_addLoadedClass() {
            ...
            setTimeout((() => {
                ...
                if (htmlDucument.closest(".loaded")) if (script_innerWidth >= 767.98) {
                    qea(".menu__list li").forEach((item => {
                        item.style.cssText = "opacity: 0; visibility: hidden; transform: translateY(20px)";
                    }));
                    qea(".header__contacts a").forEach((item => {
                        item.style.cssText = "opacity: 0; visibility: hidden; transform: translateY(20px)";
                    }));
                    styles(".wrapper", "transition-property", "opacity");
                    styles(".wrapper", "transition-duration", "700ms");
                    styles(".wrapper", "opacity", 1);
                    setTimeout((() => {
                        styles(".wrapper", "", "", "transition");
                        qe(".header__container").classList.add("visible");
                        qe(".header__buttons").classList.add("visible");
                        qe(".header__logo").classList.add("visible");
                        setTimeout((() => {
                            let dur = 0;
                            qea(".menu__list li").forEach(((item, i) => {
                                styles(item, "transition-duration", "300ms");
                                styles(item, "transition-property", "visibility,opacity,transform");
                                styles(item, "transition-delay", `${40 * i + "ms"}`);
                                styles(item, "opacity", "1");
                                styles(item, "visibility", "visible");
                                styles(item, "transform", "translateY(0px)");
                                dur = 40 * i + 40;
                                setTimeout((() => {
                                    styles(item, "", "", "transition");
                                    styles(item, "", "", "opacity");
                                    styles(item, "", "", "visibility");
                                    styles(item, "", "", "transform");
                                }), 300 + dur);
                            }));
                            setTimeout((() => {
                                qea(".header__contacts a").forEach(((item, i) => {
                                    let dur = 40 * i + 40;
                                    styles(item, "transition-duration", "300ms");
                                    styles(item, "transition-property", "visibility,opacity,transform");
                                    styles(item, "transition-delay", `${40 * i + "ms"}`);
        ...
        if (sections.length) {
            ...
            function calcScrProc(scr, name) {
                let result = scr / qe(`${name}`).offsetHeight * 100;
                ...
                setTimeout((() => {
                    ...
                    function actionsScroll(e) {
                        ...
                        if (scroll > qe("#sc2").offsetHeight + qe("#sc2").offsetTop && scroll < qe("#sc4").parentElement.offsetTop) {
                            ...
                            let scrProc = calcScrProc(scr, "#sc2");
                            ...
                            if (scrProc > 50 && scrProc < 100) qe(".introduction__description").style.removeProperty("transform");
                            if (scrProc > 30 && scrProc < 60) qe(".introduction__btn-box").style.transform = `translateY(${h / 1.5 - h / 1.5 / 100 * (3.33 * (scrProc - 30)) + "px"})`;
                            if (scrProc > 60 && scrProc < 100) qe(".introduction__btn-box").style.removeProperty("transform");
                            ...
                            if (scrProc > 100 && scrProc < 225) {
                                ...
                                if (scrProc > 100 && scrProc < 125) {
                                    qe(".introduction__description").style.transform = `translateY(${0 + h / 1.5 / 25 * (2 * (scrProc - 100)) + "px"})`;
                                    qe(".introduction__btn-box").style.transform = `translateY(${0 + h / 1.5 / 25 * (2 * (scrProc - 100)) + "px"})`;
                                }
                                qe(".introduction__title").style.transform = `scale(${easeInQuart(scrProc - 100, 1, 599, 100)})`;
                                qe(".introduction__title-block span").style.transform = `scale(${easeInQuart(scrProc - 100, 1, 599, 100)}) translate(-50%,-50%)`;
                            }
                            function easeInQuart(t, b, c, d) {
                                let current = c * (t /= d) * t * t * t + b;
                                return current = parseFloat(current.toFixed(2));
                            }
                            if (scrProc < 100) {
                            ...
                            if (scrProc > 110 && scrProc < 250) if (scrProc > 135 && scrProc < 170) {
                                qe(".introduction__rect").setAttribute("fill", `rgba(0,0,0,${1 - 1 / 35 * (scrProc - 135)})`);
                                qe("#sc2").style.backgroundColor = `rgba(255,255,255,${1 - 1 / 35 * (scrProc - 135)})`;
                            ...
                            if (scrProc > 225 && scrProc < 250) {
                                ...
                                qe("#sc3").style.position = "fixed";
                                qe(".introduction__title").style.opacity = 0;
                            }
                            if (scrProc < 225) {
                                qe(".introduction__title").style.opacity = 1;
                                qe("#sc3").style.zIndex = -1;
